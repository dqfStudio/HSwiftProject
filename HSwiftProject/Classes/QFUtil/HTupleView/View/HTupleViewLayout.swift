...
private class HTupleReusableView: UICollectionReusableView {
    override func apply(_ layoutAttributes: UICollectionViewLayoutAttributes) {
        super.apply(layoutAttributes)
        guard let attr = layoutAttributes as? HTupleViewLayoutAttributes,
...
@objc protocol HTupleViewLayoutDelegate: UICollectionViewDelegateFlowLayout {
...
class HTupleViewLayout: UICollectionViewFlowLayout {
    ...
    override func prepare() {
        ...
        guard let delegate = collectionView.delegate as? HTupleViewLayoutDelegate else { return }

        // 1. Initialization
        self.register(HTupleReusableView.self, forDecorationViewOfKind: HTupleViewSectionColor)
        ...
        for section in 0..<sections {
            ...
            // 2. Definition
            let attr: HTupleViewLayoutAttributes = HTupleViewLayoutAttributes(forDecorationViewOfKind: HTupleViewSectionColor, with: IndexPath(row: 0, section: section))
