...
///三个label横向从左向右抱紧显示
class HCollViewApexHoriValue1: HCollTmplApex {
    ...
    override func relayoutSubviews() {
        ...
        if let imageView = _imageView {
            ...
            updateApexConstraint(&imageWidthConstraint, on: imageView, attr: .width, constant: imageFrame.width)
            updateApexConstraint(&imageHeightConstraint, on: imageView, attr: .height, constant: imageFrame.height)
        ...
        updateApexConstraint(&labelWidthConstraint, on: label, attr: .width, constant: actualLabelWidth)
        if !didSetupLayout {
        ...
        if let accsryLabel = _accsryLabel {
            ...
            updateApexConstraint(&detailLabelWidthConstraint, on: detailLabel, attr: .width, constant: actualDetailWidth)
            if !didSetupLayout {
        ...
        if let detailView = _detailView {
            ...
            updateApexConstraint(&detailWidthConstraint, on: detailView, attr: .width, constant: detailFrame.width)
            updateApexConstraint(&detailHeightConstraint, on: detailView, attr: .height, constant: detailFrame.height)
        ...
        if isShowAccsryArrow {
            ...
            updateApexConstraint(&accsryWidthConstraint, on: accsryView, attr: .width, constant: 7)
            updateApexConstraint(&accsryHeightConstraint, on: accsryView, attr: .height, constant: 13)
        }
...
///三个label横向从右向左抱紧显示
class HCollViewApexHoriValue2: HCollTmplApex {
    ...
    override func relayoutSubviews() {
        ...
        if let detailLabel = _detailLabel {
            if detailWidth > 0 {
                updateApexConstraint(&detailLabelWidthConstraint, on: detailLabel, attr: .width, constant: detailWidth)
            }
            if !didSetupLayout {
                textLayoutView.addArrangedSubview(detailLabel)
            }
        ...
        if let accsryLabel = _accsryLabel {
            if accsryWidth > 0 {
                updateApexConstraint(&accsryLabelWidthConstraint, on: accsryLabel, attr: .width, constant: accsryWidth)
            }
            if !didSetupLayout {
                textLayoutView.addArrangedSubview(accsryLabel)
            }
...
///三个label纵向显示
class HCollViewApexHoriValue3: HCollTmplApex {
...
private func updateApexConstraint(_ constraint: inout NSLayoutConstraint?, on view: UIView, attr: NSLayoutConstraint.Attribute, constant: CGFloat) {
