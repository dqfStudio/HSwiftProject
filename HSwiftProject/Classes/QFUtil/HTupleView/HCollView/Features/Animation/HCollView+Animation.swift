...
extension HCollView {
    ...
    /// 入场动画。delay 按 item 递增，上限 0.24s。建议在 `willDisplayCell` 里调用。
    func playAppearAnimation(_ animation: CellAppearAnimation, on cell: UICollectionViewCell, at indexPath: IndexPath) {
        let delay = min(TimeInterval(indexPath.item) * 0.04, 0.24)
    ...
    func playAppearAnimation(_ animation: CellAppearAnimation, on cells: [UICollectionViewCell]) {
        for (index, cell) in cells.enumerated() {
            playAppearAnimation(animation, on: cell, at: IndexPath(item: index, section: 0))
        }
    }
}
