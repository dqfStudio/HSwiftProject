...
func HCollRectIntegral(_ rect: CGRect) -> CGRect {
...
func HCollSizeIntegral(_ size: CGSize) -> CGSize {
    CGSize(width: floor(size.width), height: floor(size.height))
}

extension HCollView {
    /// 仍指向当前数据源的 item。section / item 越界的会丢掉。
    internal func validItemIndexPaths(from indexPaths: [IndexPath]) -> [IndexPath] {
        let sections = numberOfSections
        return indexPaths.filter { path in
            path.section >= 0
                && path.section < sections
                && path.item >= 0
                && path.item < numberOfItems(inSection: path.section)
        }
    }
}
