...
extension HFlowView {
    ...
    @objc func hflow_memory_release() {
        releaseFlowBlock()
    }

    /// 内存警告：清 identifier 登记和图片内存缓存。identifier 清掉后下次会重新 register。
    @objc func handleMemoryWarning() {
        invokeFeature(HFlowFeatureSelector.imageSizeClearCache)
        invokeFeature(HFlowFeatureSelector.prerenderClear)
    ...
    func releaseFlowBlock() {
        clearFlowState()
        flowDelegate = nil
        invokeFeature(HFlowFeatureSelector.refreshClearBlocks)
    }
