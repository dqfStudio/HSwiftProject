...
class HFlowRowApexChrome: HFlowStackApex {
    ...
    /// 对应 HCollRowApex.imageView；与 Cell 侧 `contentImageView` 同义（HeaderFooter 无系统 imageView 冲突，仍用独立命名保持 Cell/Apex 一致）。
    var contentImageView: HImageTextView {
        if !usesImage {
            usesImage = true
            setNeedsLayout()
        }
        if let imageView = _contentImageView { return imageView }
    ...
    override func relayoutSubviews() {
        ...
        syncArranged([
            usesImage ? _contentImageView : nil,
            textLayoutView,
        ...
        if usesImage, let imageView = _contentImageView {
        ...
        setSpacing(spacingAfterImage, after: usesImage ? _contentImageView : nil)
    ...
    override func prepareForReuse() {
        ...
        _contentImageView?.resetForReuse()
        ...
        detailImageWidthConstraint?.isActive = false
        detailImageHeightConstraint?.isActive = false
        arrowWidthConstraint?.isActive = false
        arrowHeightConstraint?.isActive = false
...
/// 文案从左抱紧，中间没有弹性空位。
class HFlowPackedRowApex: HFlowRowApexChrome {
...
/// 标题在左，详情/附加靠右。
class HFlowValueRowApex: HFlowRowApexChrome {
...
/// 中间标题 / 副标题 / 附加竖着叠。
class HFlowSubtitleRowApex: HFlowRowApexChrome {
