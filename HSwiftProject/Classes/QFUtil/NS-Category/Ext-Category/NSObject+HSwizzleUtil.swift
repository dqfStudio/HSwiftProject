...
func HSwizzleClassMethod(_ cls: AnyClass, _ origSEL: Selector, _ overrideSEL: Selector) {
    if let originalMethod = class_getClassMethod(cls, origSEL),
        let swizzledMethod = class_getClassMethod(cls, overrideSEL),
        let classString = NSStringFromClass(cls).cString(using: .utf8) {
        let metacls: AnyClass = objc_getMetaClass(classString) as! AnyClass
        if (class_addMethod(metacls,
            ...
            if let classMethod = class_replaceMethod(metacls,
                                                     origSEL,
                                                     method_getImplementation(swizzledMethod),
                                                     method_getTypeEncoding(swizzledMethod)) {
                class_replaceMethod(metacls,
...
func HSwizzleClassMethodNames(_ classNames: NSArray, _ origSEL: Selector, _ overrideSEL: Selector) {
    classNames.forEach { className in
        if let classString = NSClassFromString(className as! String) {
            HSwizzleClassMethod(classString, origSEL, overrideSEL)
        }
    }
}
