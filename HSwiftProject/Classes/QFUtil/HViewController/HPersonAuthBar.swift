...
typealias HPersonAuthBarBlock = (_ auth: String) -> Void

class HPersonAuthBar: UIStackView, HTupleViewDelegate {
    ...
    private func sortAuth(_ auth: String) -> [String] {
        ...
        authString = authString.replacingOccurrences(of: "GM", with: "")
        authString = authString.replacingOccurrences(of: "VIP", with: "")
        guard !authString.isEmpty else { return [] }
        let items = authString.components(separatedBy: ",")
        let order = ["POP", "OTC", "UGC"]
